...
const MobileNavbar = ({ token, setShowSearch, getCartCount }) => {
  ...
  const navItems = [
    ...
    { icon: Play, label: "Collection", path: "/collection", ariaLabel: "Navigate to Collection" },
    { icon: Grid, label: "Wishlist", path: "/wishlist", ariaLabel: "Navigate to Wishlist" },
    {
      icon: ListOrdered,
      label: "Order",
      path: token ? "/order" : "/login",
      ariaLabel: token ? "Navigate to Orders" : "Navigate to Login",
    },
  ];
